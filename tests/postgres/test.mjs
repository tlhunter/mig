...

{
    ...
    const addEmailToUsers = stdout[1];
    assert.equal(Object.keys(addEmailToUsers).length, 2, '[0] has two keys')
    assert.equal(Object.keys(addEmailToUsers.migration).length, 4, '[0].migration has four keys')
    assert.equal(addEmailToUsers.migration.id, 2, '[0].migration.id is correct')
    assert.equal(addEmailToUsers.migration.name, '20230101120107_add_email_to_users.sql', '[0].migration.name is correct')
    assert.equal(addEmailToUsers.migration.batch, 2, '[0].migration.batch is correct')
    assert.equal(typeof addEmailToUsers.migration.time, 'string', '[0].migration.time is present')
    assert.equal(addEmailToUsers.status, 'applied', '[0].status is correct')
}
